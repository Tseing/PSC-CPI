...
train_loader = torch.utils.data.DataLoader(
    dataset=train_set, batch_size=param["batch_size"], shuffle=True, pin_memory=True, num_workers=6
)
val_set = data_loader(name_split="val")
val_loader = torch.utils.data.DataLoader(
    dataset=val_set, batch_size=param["batch_size"], shuffle=False, pin_memory=True, num_workers=6
)
    ...
    for epoch in range(pre_epochs):
        ...
        for (
            ...
            (
                ...
                prot_data.to(device, non_blocking=True),
                drug_data_ver.to(device, non_blocking=True),
                drug_data_adj.to(device, non_blocking=True),
                prot_contacts.to(device, non_blocking=True),
                prot_inter.to(device, non_blocking=True),
                prot_inter_exist.to(device, non_blocking=True),
        ...
        if epoch % 10 == 0 or epoch <= 10:
            logger.warning(
                "Pre-training Epoch: {}, Train loss: {}".format(epoch, loss_epoch / batch)
            )
    ...
    for epoch in range(param["epoch"]):
        ...
        for (
            ...
            (
                ...
                prot_data.to(device),
                drug_data_ver.to(device),
                drug_data_adj.to(device),
                prot_contacts.to(device),
                prot_inter.to(device),
                prot_inter_exist.to(device),
                label.to(device),
            )
            with torch.no_grad():
                loss = model(
        ...
        if epoch % 10 == 0 or epoch <= 10:
            logger.warning(
                "Epoch: {}, Train loss: {} | Val loss: {}".format(
                    epoch, loss_epoch / batch, loss_epoch_val / batch_val
                )
            )
...
eval_loader = torch.utils.data.DataLoader(
    dataset=eval_set, batch_size=param["batch_size"], shuffle=False, pin_memory=True, num_workers=4
)
...
auprc, auroc = cal_interaction_torch(
    model, eval_loader, prot_length, comp_length, param["batch_size"]
)
...
eval_loader = torch.utils.data.DataLoader(
    dataset=eval_set, batch_size=param["batch_size"], shuffle=False, pin_memory=True, num_workers=6
)
...
print(
    "Unseen Prot | RMSE: {} | Pearson: {} | AUPRC: {} | AUROC: {}".format(
        rmse, pearson, auprc, auroc
    )
)
results.append(str(rmse))
results.append(str(pearson))
...
print(
    "Unseen Comp | RMSE: {} | Pearson: {} | AUPRC: {} | AUROC: {}".format(
        rmse, pearson, auprc, auroc
    )
)
results.append(str(rmse))
...
print(
    "Unseen Both | RMSE: {} | Pearson: {} | AUPRC: {} | AUROC: {}".format(
        rmse, pearson, auprc, auroc
    )
)
results.append(str(rmse))
